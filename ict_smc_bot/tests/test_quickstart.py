...
class TestQuickstart(unittest.TestCase):
    ...
    def test_research_runs_offline(self):
        quickstart._CLIENT_FACTORY = lambda *, provider, model, base_url: _FakeResearchClient()
        rc = quickstart.main(["research", "--csv", DATA, "--rounds", "1",
                              "--batch", "2", "--n-folds", "3"])
        self.assertEqual(rc, 0)

    def test_advisor_runs_offline_veto(self):
        quickstart._CLIENT_FACTORY = lambda *, provider, model, base_url: _FakeAdvisorClient({"action": "veto"})
        rc = quickstart.main(["advisor", "--csv", DATA])
        self.assertEqual(rc, 0)

    def test_advisor_runs_offline_downsize(self):
        quickstart._CLIENT_FACTORY = lambda *, provider, model, base_url: _FakeAdvisorClient(
